...
> **템플릿 다양성 회귀 수정 + rollup 집계 신뢰성.** 0.2.0의 셀러빌리티 셸이 모든 템플릿에 무조건 주입돼 형식이 균일해지고 'My Template' 플레이스홀더가 노출되던 회귀를, 실 Notion E2E 5종(쉬움~어려움) 생성 테스트로 발견·수정. 셸을 복잡도 적응형으로 전환해 단순=린·복잡=리치로 골격 차등 복원. rollup이 항상 실값을 집계하도록 폴백 오토링커 추가.

### Fixed
- **rollup 샘플 링크 폴백 오토링커** — AI/recipe가 샘플 relation 값을 누락하거나 대상 제목과 안 맞으면 제목 매칭 해석이 0건이 되어 rollup이 0/None으로 집계되지 않던 문제(습관 트래커 라이브 케이스). rollup을 먹이는 relation에 한해, 한 줄도 연결 안 됐을 때만 대상 샘플을 라운드로빈 배정해 반드시 집계되게 함. AI가 제대로 준 링크는 보존. 결정적 단위테스트 3종.
- **'My Template' 플레이스홀더 누출** — AI가 제목을 main_page.title에만 담아 보내면 후처리가 top-level title만 읽어 환영 callout/quote에 'My Template'이 그대로 노출되던 결함. 제목을 top-level/main_page/metadata 순으로 해석하도록 수정(post_processor·blueprint_generator 양쪽).
- **generic '데이터베이스 N' DB명** — AI가 DB title을 누락하고 역방향 relation도 없을 때 generic 폴백이 노출되던 문제. DB명을 description 핵심 명사·구별되는 title 속성명에서도 유추하도록 강화(출판사 케이스: 작가/책/판매). 결정적 단위테스트 6종.

### Changed
- **셀러빌리티 셸 적응형화 (다양성 복원)** — 0.2.0에서 온보딩 페이지·상단 네비·목차를 모든 템플릿에 무조건 주입해 형식이 균일해지던 회귀 수정. 이제 온보딩은 멀티 DB 템플릿에만, 네비는 하위페이지 ≥2, 목차는 heading ≥3일 때만 적용. 단순 템플릿은 린하게, 복잡 템플릿은 리치하게 골격이 달라짐.
- **후처리 환영 callout 중복 제거** — 상단 3블록에 callout이 이미 있으면 AI 인트로를 존중하고 제네릭 환영 callout을 추가하지 않음.
- **`metadata.layout` 기록** — 라우팅된 레이아웃을 metadata에 기록(전엔 None) → 관측·검증·프론트 반영 가능.

### 실데이터 검증 (실 Notion E2E)
- 쉬움~어려움 5종 재생성: 독서(1DB·8블록·린) ↔ 출판사(3DB·4서브·linked_view 5·리치)로 골격 차등 복원, **My Template 0건**, layout 전부 기록, rollup 집계 정상(습관 [2,2,2,2]·CRM [450만…]·OKR 진행률 [100,70,100,22.5]), DB명 작가/책/판매.
- 백엔드 **1,556** 테스트 통과(신규 11: 폴백 3·DB명 6·sellability 음성 2, 적응형 갱신 포함), ruff clean.

---

## [0.2.0] - 2026-06-02

> **"유료급 품질" 대규모 업그레이드.** 품질 측정·게이트(루브릭 + LLM judge + judge→repair), 셀러빌리티 보강(온보딩·상단네비·리스팅킷), 시각 프리미엄(뷰 큐레이션·아이콘), 도메인 예시 주입, 프론트 품질신호 표시, AI 대화형 수정(recolor), 버전 히스토리, 토큰 집계 실측, 위생·정직화(死코드·하드코딩·README). **실 Notion E2E로 $50-99~$100+ 멀티DB 템플릿 생성·rollup 집계 입증.** 백엔드 1,545 테스트·프론트 build/smoke green.
...
### Added (Phase 5 — C1 비용 + C2 평가, 1차)
- **토큰 집계 실측 (C1)** — claude/groq/openai가 응답 usage를 `note_tokens`로 누적 → `metrics.finish`가 세션 토큰 기록. **/metrics·summary가 항상 0이던 토큰 메트릭을 실측화**(Phase 2 이월 정직화 완료). 라이브: 생성 1회 = **4,706 토큰 집계** 확인.
- **품질 회귀 게이트 (C2)** — recipe 코퍼스가 품질 밴드 아래로 떨어지면 CI 실패(advanced crm/okr ≥ 75, 그외 ≥ 40). 엔진/루브릭 변경의 품질 회귀 자동 차단.
