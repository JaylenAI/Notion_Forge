...
> 최종 업데이트: 2026-06-03
> 현재 브랜치: feature/template-differentiation (v0.2.1 — dev 머지 대기)
> 버전: **v0.2.1** (템플릿 다양성 회귀 수정 + rollup 집계 신뢰성 — 2026-06-03)

---

## v0.2.1 — 템플릿 다양성 회귀 수정 (2026-06-03)

실 Notion E2E 5종(쉬움~어려움) 생성 테스트에서 **모든 템플릿이 같은 골격으로 보이던 회귀**를 발견·수정.
- 원인: 0.2.0의 셀러빌리티 셸(온보딩/네비/목차)이 모든 템플릿에 무조건 주입 + 후처리 환영 callout/quote의 'My Template' 플레이스홀더 누출.
- 수정: 셸을 **복잡도 적응형**으로(단순=린, 멀티DB=리치) + 제목 해석을 main_page까지 확장(My Template 0건) + `metadata.layout` 기록 + generic DB명 유추 강화(description/title 속성).
- rollup 폴백 오토링커로 **rollup 항상 실값 집계** 보장.
- 검증: 독서(1DB·린) ↔ 출판사(3DB·리치) 골격 차등, rollup 집계 정상(습관/CRM/OKR), DB명 작가/책/판매. 백엔드 **1,556** 테스트·ruff clean.
